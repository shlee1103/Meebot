...
interface PresentationModalProps {
  ...
  participants: string[];
  onSpeakerSelect: (participant: string) => void;
              ...
              {selectedSpeakers.length > 0 ? (
                <div className="flex flex-wrap gap-2">
                  {selectedSpeakers.map((speaker) => (
                    <div key={speaker} className="bg-[#2C3440] text-white px-3 py-1 rounded-full flex items-center gap-2">
                      <span>{speaker}</span>
                      <button onClick={() => onSpeakerRemove(speaker)} className="text-gray-400 hover:text-white">
                        ×
                      </button>
                    </div>
                  ))}
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {participants
                  ...
                  .map((participant) => (
                    <button key={participant} onClick={() => onSpeakerSelect(participant)} className="bg-[#2C3440] text-white px-3 py-1 rounded-full hover:bg-[#3C4450]">
                      {participant}
                    </button>
                  ))}
              </div>
