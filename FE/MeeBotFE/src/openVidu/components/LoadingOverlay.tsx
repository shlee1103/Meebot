...
const LoadingOverlay: React.FC<LoadingOverlayProps> = ({ onLoadingComplete }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      const remainingSeconds = Math.max(5 - Math.floor(elapsedTime / 1000), 0);
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-md z-50 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative text-center space-y-16">
        ...
        <FloatingText>
          <H1 className="text-white mb-4 animate-pulse text-9xl drop-shadow-[0_0_10px_rgba(255,255,255,0.7)]">
            {countdown}
          </H1>
        </FloatingText>
        ...
        <div className="space-y-2">
          <P className="text-white text-2xl tracking-wider font-semibold drop-shadow-[0_0_8px_rgba(94,234,212,0.5)]">
            곧 발표회가 시작될 예정입니다
          </P>
          <Mn className="text-teal-200">
            MeeBot과 함께하는 특별한 순간
          </Mn>
        </div>
