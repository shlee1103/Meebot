...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (requestURI.equals("/api/v1/oauth2/google") ||    // 구글 로그인 관련 (GET, POST)
                ...
                requestURI.startsWith("/api/chatgpt") || requestURI.startsWith("/api/notion/login") || requestURI.startsWith("/api/auth/notion/callback") | requestURI.startsWith("/api/download/pdf")) {         // ChatGPT 관련 API (하위 경로 모두 포함)
